...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Cillocoin users should be well used to small numbers with many decimal places by now :)
        ...
        for (CNode* pnode : vNodesCopy)
        {
            ...
            if (recvSet || errorSet)
            {
                ...
                else if (nBytes == 0)
                {
                    ...
                    if (!pnode->fDisconnect) {
			LogPrint(BCLog::NET, "**ME** socket closed 'gracefully'\n");
                    }
                    pnode->CloseSocketDisconnect();
                }
                else if (nBytes < 0)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Cillocoin " + FormatFullVersion();
