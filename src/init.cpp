 ...
 * between cillocoind, and cillocoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("cillocoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), CILLOCOIN_CONF_FILENAME));
    if (mode == HMM_CILLOCOIND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), CILLOCOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/cillocoin/cillocoin>";
    const std::string URL_WEBSITE = "<https://cillocoincore.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("cillocoin-loadblk");
...
/** Sanity checks
 *  Ensure that Cillocoin is running in a usable environment with all
...
{
    // Make sure only a single Cillocoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", CILLOCOIN_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if cillocoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if cillocoin is started while in a temporary directory.\n",
