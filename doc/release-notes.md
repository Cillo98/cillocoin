Cillocoin Core version 0.16.0 is now available from:

  <https://cillocoincore.org/bin/cillocoin-core-0.16.0/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/cillocoin/cillocoin/issues>

To receive security and update notifications, please subscribe to:

  <https://cillocoincore.org/en/list/announcements/join/>
...
installer (on Windows) or just copy over `/Applications/Cillocoin-Qt` (on Mac)
or `cillocoind`/`cillocoin-qt` (on Linux).
...
Cillocoin Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

Cillocoin Core should also work on most other Unix-like systems but is not
...
Cillocoin Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
cillocoin data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Cillocoin Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µCLC" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- cillocoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- cillocoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 15001.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest cillocoin.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.cillocoin.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for cillocoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to cillocoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #10825 `28485c7` Set regtest JSON-RPC port to 18443 to avoid conflict with testnet 15001 (fametrano)
...
- #12035 `eeb6d52` Change µCLC to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_cillocoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` cillocoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.cillocoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for cillocoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in cillocoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop cillocoind (OmeGak)
...
- #10529 `331352f` Improve cillocoind systemd service file (Flowdalic)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/cillocoin/).
