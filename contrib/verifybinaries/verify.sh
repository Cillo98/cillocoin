...
###   cillocoincore.org and cillocoin.org and compares them.
...
WORKINGDIR="/tmp/cillocoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://cillocoincore.org"
HOST2="https://cillocoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="cillocoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'cillocoin-',
...
if [ $? -ne 0 ]; then
   echo "cillocoin.org failed to provide signature file, but cillocoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "cillocoin.org and cillocoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Cillocoin Core binary release signing key installed?"
   fi
