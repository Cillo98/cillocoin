...
The RPM spec file provided here is for Cillocoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://cillocoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Cillocoin git repository](https://github.com/cillocoin/cillocoin) but are not part of
...
Currently only `Source10` is used, for the example `cillocoin.conf` file.
...
in the cillocoin git repository. At present only an SVG version of the Cillocoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the cillocoin
...
needed when 0.12.1 is released, a proper fix is already in the Cillocoin git
master branch.
...
idea to build Cillocoin Core against a static version of that library compiled
...
The `cillocoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `cillocoin-server` package. At this point, most RPM based
...
The `cillocoin-server` package should have SELinux support. How to properly do
...
Cillocoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/cillocoin/). He has been
packaging Cillocoin for Fedora at least since 2012.
...
4. I renamed the `cillocoin` package that contains the Qt GUI to `cillocoin-core` as
that appears to be how the general population refers to it, in contrast to
`cillocoin-xt` or `cillocoin-classic`. I wanted to make sure the general population
